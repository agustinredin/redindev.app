...
const LinkButton = ({ icon, title, href, _bgColor, _textColor }) => {
    ...
    return (
        <chakra.Link href={href} target='_blank' textDecoration={'none !important'} w={{base: '100%', md: 'auto'}}>
            <chakra.Tooltip label={tooltipText}>
                <chakra.Flex w='100%' h='100%' _hover={{ bgColor: `${_textColor}`, color: `${_bgColor}`, border: `1px solid ${_bgColor}`, transition: 'all 0.2s', transform: 'translateY(-5px)' }}
                    p='0.5rem'
